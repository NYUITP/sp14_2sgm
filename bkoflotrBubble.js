(function () {
    'use strict';
    beaker.bkoDirective("BasicBubble20", function () {
 return {
	...
	link: function (scope, element, attrs) {	
		...
		var intervalX = largestX/xcount;
		var intervalY = largestY/ycount;

		var container = document.getElementById('container');
		var d1 = [],d2 = [],point, graph, i;
		      console.log("before for loop");
		  for (i = 0; i < count; i++){
      	    		    console.log(arrCol1[i][1]);
			    point = [i,arrCol1[i][1],arrCol2[i][1],(arrCol3[i][1])/100];
			    console.log(point);
			    d1.push(point);
    			   // d2.push(point);
		  }
		  ...
		  graph = Flotr.draw(container, [d1], {
		    ...
		    xaxis   : {min: smallestX - largestX, max: largestX + intervalX, count: xcount},
		    yaxis   : {min: smallestY - largestY, max: largestY + intervalY, count: ycount}
		});
       }
     };
    });
